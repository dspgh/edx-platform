...
from xmodule.modulestore.tests.utils import (add_temp_files_from_dict,
    remove_temp_files_from_dict, DOT_FILES_DICT, TILDA_FILES_DICT)
...
class IgnoredFilesTestCase(unittest.TestCase):
    ...
    def teardown_method(self, method):
        for dictionary in self.dict_list:
            remove_temp_files_from_dict(dictionary.keys(), self.course_dir / "static")
    ...
    def test_sample_static_files(self):
        ...
        """
        # Add sample files to course dir
        for dictionary in self.dict_list:
